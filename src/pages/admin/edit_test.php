...
<head>
    ...
    <title>Oxford CEFR Speaking - Edit Test</title>
    ...
    <main class="flex-1 overflow-y-auto bg-gray-50 p-6">
        <div class="container mx-auto max-w-5xl">
            ...
            <?php if (isset($success)): ?>
                <div class="bg-green-100 border-l-4 border-success text-success p-4 mb-6 rounded shadow-sm">
            ...
            <div class="bg-white p-6 rounded-lg shadow mb-8 border border-gray-200">
                ...
                <form method="POST" class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div class="md:col-span-1">
                        ...
                        <input class="w-full p-2 rounded border border-gray-300 focus:ring-2 focus:ring-primary focus:outline-none" type="text" name="title" value="<?php echo htmlspecialchars($test['title']); ?>" required>
                    </div>
                    <div class="md:col-span-2">
                        <label class="block text-textMuted font-medium mb-2" for="description">Description</label>
                        <div class="flex gap-2">
                            <input class="w-full p-2 rounded border border-gray-300 focus:ring-2 focus:ring-primary focus:outline-none" type="text" name="description" value="<?php echo htmlspecialchars($test['description']); ?>">
                            <button type="submit" name="update_test" class="bg-secondary hover:bg-gray-700 text-white px-6 py-2 rounded transition font-medium">Save</button>
                        </div>
                    </div>
                </form>
            </div>
            ...
            <div class="space-y-8 pb-10">
                <?php foreach ($partTitles as $partType => $title): ?>
                    <div class="bg-white rounded-lg shadow overflow-hidden border border-gray-200">
                        <div class="bg-gray-50 p-4 border-b border-gray-200 flex justify-between items-center sticky top-0 z-10">
                            <h3 class="font-bold text-lg text-textMain"><?php echo $title; ?></h3>
                            <a href="edit_question.php?test_id=<?php echo $testId; ?>&part=<?php echo $partType; ?>" class="bg-white border border-primary text-primary hover:bg-blue-50 px-3 py-1 rounded text-sm font-semibold transition shadow-sm">
                                + Add Question
                            </a>
                        </div>
                                ...
                                <?php foreach ($questionsByPart[$partType] as $q): ?>
                                    <div class="p-4 hover:bg-gray-50 transition flex justify-between items-center group">
                                        <div class="flex-grow pr-4">
                                            ...
                                            <p class="text-textMain font-medium truncate max-w-2xl">
                                                <?php
                                                    $content = $q['content'];
                                        ...
                                        <div class="flex space-x-2">
                                            <a href="edit_question.php?test_id=<?php echo $testId; ?>&question_id=<?php echo $q['id']; ?>" class="text-primary hover:text-primaryHover font-medium text-sm p-2 border border-transparent hover:border-primary rounded transition">Edit</a>
                                            <a href="delete_question.php?test_id=<?php echo $testId; ?>&question_id=<?php echo $q['id']; ?>" class="text-danger hover:text-red-700 font-medium text-sm p-2 border border-transparent hover:border-danger rounded transition" onclick="return confirm('Delete this question?')">Delete</a>
                                        </div>
                                    </div>
                                <?php endforeach; ?>
                            <?php endif; ?>
